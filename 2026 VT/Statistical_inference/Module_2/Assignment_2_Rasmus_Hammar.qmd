...
plt.vlines(7, ymin=0, ymax=150, colors="red", linestyles="dashed")
...
With `{python} "p = {:.3f}".format(guessing_pvalue)` I would reject the null hypothesis, but most likely consider the model subpar.
...
simulations_pvalue = (len([s for s in simulations if s <= 0.75])) / len(simulations)
```
...
plt.vlines(simulations_ci[0], ymin=0, ymax=100, colors="blue", linestyles="dashed")
plt.vlines(simulations_ci[1], ymin=0, ymax=100, colors="blue", linestyles="dashed")
...
plt.vlines(0.75, ymin=0, ymax=300, colors="red", linestyles="dashed")
...
B_mean_diff_pvalue = len([d for d in B_mean_diff if abs(d) >= abs(H1_diff)]) / len(
    B_mean_diff
)
```
...
plt.vlines(B_mean_diff_ci[0], ymin=0, ymax=800, colors="blue", linestyles="dashed")
plt.vlines(B_mean_diff_ci[1], ymin=0, ymax=800, colors="blue", linestyles="dashed")
...
print("Difference in mean IQR between species A and B: {:.3f}".format(d_mean_IQR))
```
...
B_mean_diff_ci = np.percentile(B_mean_diff, [2.5, 97.5])
B_mean_diff_pvalue = len([d for d in B_mean_diff if abs(d) >= abs(d_mean_IQR)]) / len(B_mean_diff)
```
