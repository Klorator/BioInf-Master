...
-- PRODUCT: common lookups, filters and joins
...
-- If you often filter by department AND price together, use a composite:
CREATE INDEX idx_product_dep_price   ON product (dep_name, prod_price);

-- KEYWORDS: search keyword -> product
CREATE INDEX idx_keywords_keyword    ON keywords (keyword);

-- REVIEW: lookups by product, by customer, by date
...
-- CUSTOMER: lookups by last name / phone (email is UNIQUE -> already indexed)
CREATE INDEX idx_customer_lastname   ON customer (last_name);
CREATE INDEX idx_customer_phone      ON customer (phone_nr);

-- ORDERS: find orders by customer and by status+date (useful for dashboards)
CREATE INDEX idx_orders_cust         ON orders (cust_id);
CREATE INDEX idx_orders_status_date  ON orders (status, date_created);

-- PROD_ORDERED: primary key (order_id, prod_id) exists, add index to find orders for a product
